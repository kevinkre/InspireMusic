...
class TransformerLM(torch.nn.Module):
    def __init__(
        ...
        self.chorus_embedding = torch.nn.Embedding(5, llm_input_size)
        ...
        self.speech_embedding = torch.nn.Embedding(4, llm_input_size)
    ...
    def pad_unpad_sequence(self, sos_eos_emb, embeddings, text_token, text_token_len, task_id_emb, speech_token, speech_token_len):
        text_token = unpad_sequence(text_token, text_token_len.cpu(), batch_first=True)
        speech_token = unpad_sequence(speech_token, speech_token_len.cpu(), batch_first=True)

        lm_input = [torch.concat([sos_eos_emb.squeeze(dim=0)] + [embedding[i] for embedding in embeddings] + [text_token[i], task_id_emb.squeeze(dim=0), speech_token[i]], dim=0)
    ...
    def forward(
        ...
        lm_target = [torch.tensor([IGNORE_ID] * (4 + text_token_len[i]) + speech_token[i, :speech_token_len[i]].tolist() +
        ...
        time_start_embed = self.time_embedding(time_start)
        time_end_embed = self.time_embedding(time_end)
    ...
    def inference(
        ...
        # Check if removing prompt speech token will fail decoding.

        lm_input = torch.concat([sos_eos_emb,  time_start_embed, time_end_embed, chorus_embed, text, task_id_emb], dim=1)

        # 4. cal min/max_length
        min_len = duration_to_gen * 70
        max_len = duration_to_gen * 70
