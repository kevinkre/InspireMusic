...
class LLM(torch.nn.Module):
    ...
    def forward(
        ...
        if mask:
            ...
            lm_target = [torch.tensor([IGNORE_ID] * (1 + 3 * seg_len[i] + text_token_len[i]) + audio_token[i, :audio_token_len[i]].tolist() +
    ...
    def inference(
        ...
        if task=="continuation":
            ...
            if infer_cfg:
                ...
                lm_cf_input = torch.concat([sos_eos_emb, torch.rand_like(time_start_embed), torch.rand_like(time_end_embed),
                                torch.rand_like(chorus_embed), text_cfg, task_id_emb, audio_cfg], dim=1)
        ...
        for i in range(int(max_len)):
            y_pred, _, state = self.llm.forward_one_step(lm_input, torch.ones(
                    lm_input.shape[0], lm_input.shape[1],
