...
class InspireMusicModel:
	...
	def load(self, llm_model, flow_model, hift_model, wavtokenizer_model):
		if llm_model is not None:
			self.llm.load_state_dict(
				torch.load(llm_model, map_location=self.device))
			self.llm.to(self.device).eval()
		else:
			self.llm = None
		if flow_model is not None:
			self.flow.load_state_dict(
				torch.load(flow_model, map_location=self.device))
		...
		if hift_model is not None:
			if ".pt" not in hift_model:
				...
				self.music_tokenizer = VQVAE(
					os.path.dirname(hift_model) + '/config.json',
		...
		if wavtokenizer_model is not None:
			if ".pt" not in wavtokenizer_model:
				self.wavtokenizer = WavTokenizer.from_pretrained_feat(
					wavtokenizer_model + '/config.yaml',
					wavtokenizer_model + '/model.pt')
			else:
				self.wavtokenizer = WavTokenizer.from_pretrained_feat(
					os.path.dirname(wavtokenizer_model) + '/config.yaml',
					wavtokenizer_model)
	...
	def load_onnx(self, flow_decoder_estimator_model):
		...
		self.flow.decoder.estimator = onnxruntime.InferenceSession(
			flow_decoder_estimator_model, sess_options=option,
			providers=providers)

	def llm_job(self, text, audio_token, audio_token_len, prompt_text,
				llm_prompt_audio_token, embeddings, uuid, duration_to_gen,
				task):
			...
			with autocast(enabled=self.fp16):
				for i in self.llm.inference(text=text.to(self.device),
											text_len=torch.tensor(
													...
													self.device),
											audio_token=audio_token.to(
													self.device),
											audio_token_len=audio_token_len.to(
													self.device),
											...
											prompt_text_len=torch.tensor(
													...
													dtype=torch.int32).to(
													self.device),
											prompt_audio_token=llm_prompt_audio_token.to(
													self.device),
											prompt_audio_token_len=torch.tensor(
													[
														llm_prompt_audio_token.shape[
															1]],
													dtype=torch.int32).to(
													self.device),
	...
	def token2wav(self, token, token_len, uuid, sample_rate, finalize=False):
		codec_embed = self.flow.inference(token=token.to(self.device), token_len=token_len.to(self.device), sample_rate=sample_rate)
	...
	def acoustictoken2wav(self, token):
		...
		codec_embed = self.music_tokenizer.quantizer.embed(
			torch.tensor(token).long().to(self.device)).cuda()
		wav = self.music_tokenizer.generator(codec_embed)
		wav = wav.squeeze(0).cpu().detach()
		return wav
	...
	def semantictoken2wav(self, token):
		...
		wav = self.wavtokenizer.to(self.device).decode(features,
													   bandwidth_id=bandwidth_id)
		wav = wav.cpu().detach()
		return wav

	def inference(self, text, audio_token, audio_token_len, text_token,
				  text_token_len, embeddings=None,
				  ...
				  prompt_audio_feat=torch.zeros(1, 0, 80), sample_rate=48000,
				  duration_to_gen=30, task="continuation", trim=True,
				  stream=False, **kwargs):
		...
		if self.llm:
			with self.lock:
				self.music_token_dict[this_uuid], self.llm_end_dict[
					this_uuid] = [], False
			p = threading.Thread(target=self.llm_job, args=(
			text_token, audio_token, audio_token_len, prompt_text,
			llm_prompt_audio_token, embeddings, this_uuid, duration_to_gen,
			task))
			p.start()
		...
		if stream is True:
			...
			while True:
				time.sleep(0.1)
				if len(self.music_token_dict[
						   this_uuid]) >= token_hop_len + self.token_overlap_len:
					...
					with self.lock:
						self.music_token_dict[this_uuid] = \
						self.music_token_dict[this_uuid][token_hop_len:]
					# increase token_hop_len for better audio quality
					token_hop_len = min(self.token_max_hop_len,
			...
			this_music_token = torch.concat(self.music_token_dict[this_uuid],
											dim=1)
			...
			if self.fast:
				if task == "reconstruct":
					...
					this_music_audio = self.acoustictoken2wav(
						token=this_music_token)
				else:
					if self.llm:
						p.join()
						print(len(self.music_token_dict[this_uuid]))
						this_music_token = torch.concat(
								self.music_token_dict[this_uuid], dim=1)
						print(this_music_token.shape)
					else:
						this_music_token = text_token

					logging.info(
						"using wavtokenizer generator without flow matching")
					this_music_audio = self.semantictoken2wav(
						token=this_music_token)
				...
				if self.llm:
					...
					if len(self.music_token_dict[this_uuid]) != 0:
						...
						print(
							f"The list of tensors is empty for UUID: {this_uuid}")
				else:
					this_music_token = text_token
				logging.info(
					f"LLM generated audio token length: {this_music_token.shape[1]}")
				logging.info(
					f"using flow matching and {self.generator} generator")

				if self.generator == "hifi":
					if (embeddings[1] - embeddings[0]) <= duration_to_gen:
						if trim:
							trim_length = (
								int((embeddings[1] - embeddings[0]) * 75))
							this_music_token = this_music_token[:, :trim_length]
							logging.info(
								f"After trimmed, generated audio token length: {this_music_token.shape[1]}")
					elif (embeddings[1] - embeddings[0]) < 1:
						logging.info(
							f"Given audio length={(embeddings[1] - embeddings[0])}, which is too short, please give a longer audio length.")

					this_music_audio = self.token2wav(token=this_music_token,
													  token_len=torch.LongTensor(
															  [
																  this_music_token.size(
																	  1)]),
													  uuid=this_uuid,
													  sample_rate=sample_rate,
													  finalize=True)
					logging.info(
						f"Generated audio sequence length: {this_music_audio.shape[1]}")
