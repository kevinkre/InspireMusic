...
class InspireMusicFrontEnd:
    ...
    def __init__(self,
        ...
        if self.use_ttsfrd:
            ...
            assert self.frd.initialize('{}/../../pretrained_models/InspireMusic-ttsfrd/resource'.format(ROOT_DIR)) is True, \
    ...
    def frontend_sft(self, text, spk_id):
        ...
        model_input = {'text': text_token, 'text_len': text_token_len, 'llm_embedding': embedding, 'flow_embedding': embedding}
        return model_input

    def frontend_zero_shot(self, text, prompt_text, prompt_speech_16k):
        text_token, text_token_len = self._extract_text_token(text)
        prompt_text_token, prompt_text_token_len = self._extract_text_token(prompt_text)
        prompt_speech_22050 = torchaudio.transforms.Resample(orig_freq=16000, new_freq=24000)(prompt_speech_16k)
    ...
    def frontend_cross_lingual(self, text, prompt_speech_16k):
        model_input = self.frontend_zero_shot(text, '', prompt_speech_16k)
    ...
    def frontend_instruct(self, text, spk_id, instruct_text):
        model_input = self.frontend_sft(text, spk_id)
